...
for i in range(len(all_As)):
	...
	query = """You will be given the history of a past experience in which you were placed in an environment and asked to give a sequence of moves to solve a problem. You were unsuccessful in solving the problem. Do not summarize your environment, but rather think about the strategy you took to attempt to complete the task. Devise a concise, new plan of moves that accounts for your mistake with reference to specific moves that you should have taken. For example, if you tried X and Y but forgot Z, then devise a plan to achieve Z with environment-specific moves. You will need this later when you are solving the same task. Give your plan after "Plan". Here are two examples:
	...
	""".format(reflexion_response.choices[0].message.content, number_message_mapping[num_disks],"A = "+str(A),"B = "+str(B),"C = "+str(C),number_target_mapping[num_disks])
	...
	for step in range(50):
		...
		if current_configuration == target_configuration:
			flag=1
			total_reward+=100
			with open('/jukebox/griffiths/people/smondal/tower of hanoi/logs/gpt4 zeroshot reflexion step by step external feedback/problem{}.log'.format(i+1), 'a') as w:
