...
for run_no in range(1,6):
	for i in range(len(all_As)):
		...
		with open('/jukebox/griffiths/people/smondal/tower of hanoi/logs/random step by step valid moves only/run{}/problem{}.log'.format(run_no,i+1), 'a') as w:
		...
		for step in range(50):
			...
			no_to_move = int(valid_move_to_take.split(" ")[1])
			source_list = valid_move_to_take.split(" ")[3]
			target_list = valid_move_to_take.split(" ")[5]
			total_reward+=-1
			with open('/jukebox/griffiths/people/smondal/tower of hanoi/logs/random step by step valid moves only/run{}/problem{}.log'.format(run_no,i+1), 'a') as w:
				w.write('{}. Move {} from list {} to list {}.\n'.format(step+1,no_to_move,source_list,target_list))
