	...
	while num_tries < 10:
		...
		while cur_try <10:
			try:
				...
				if len(actor_truncated_response)>=2:
					...
					one_move_msg = """
					You provided just one valid next move. However it is possible to provide two valid next moves from the current configuration. Consider moving different numbers that are at the rightmost end of the three lists to the other lists, such that the move satisfies Rule #2.
		...
		if len(actor_truncated_response)==2:
			...
			if move_validity1 =='yes' and move_validity2 =='yes':
				...
				""" .format(actor_truncated_response[1],"A = "+str(temp_current_configuration[0]),"B = "+str(temp_current_configuration[1]),"C = "+str(temp_current_configuration[2]), goal_config)
				num_tries+=1
				...
				""" .format(actor_truncated_response[0],"A = "+str(temp_current_configuration[0]),"B = "+str(temp_current_configuration[1]),"C = "+str(temp_current_configuration[2]), goal_config)
				num_tries+=1
			else:
				...
				""".format(actor_truncated_response[0],actor_truncated_response[1],"A = "+str(temp_current_configuration[0]),"B = "+str(temp_current_configuration[1]),"C = "+str(temp_current_configuration[2]), goal_config)
...
for i in range(3,106):
	...
	for step in range(50):
		...
		move_proposals, response, input, max_content_limit_exceed_count = actor_module_propose_two_actions(input,first_temp_current_configuration,number_target_mapping[num_disks],reduced_prompt,step,max_content_limit_exceed_count)
		...
		if (next_temp_current_configuration1 == target_configuration) or (next_temp_current_configuration2 == target_configuration):
			...
			""" .format("A = "+str(next_temp_current_configuration1[0]),"B = "+str(next_temp_current_configuration1[1]),"C = "+str(next_temp_current_configuration1[2]), number_target_mapping[num_disks])
			...
			next_move_proposals1, response1, input, max_content_limit_exceed_count = actor_module_propose_two_actions(input,next_temp_current_configuration1,number_target_mapping[num_disks],reduced_prompt,step,max_content_limit_exceed_count)
			...
			""" .format("A = "+str(next_temp_current_configuration2[0]),"B = "+str(next_temp_current_configuration2[1]),"C = "+str(next_temp_current_configuration2[2]), number_target_mapping[num_disks])
			...
			next_move_proposals2, response2, input, max_content_limit_exceed_count = actor_module_propose_two_actions(input,next_temp_current_configuration2,number_target_mapping[num_disks],reduced_prompt,step,max_content_limit_exceed_count)
	...
	# })
		...
		if current_configuration == target_configuration:
			flag=1
			total_reward+=100
			with open('/jukebox/griffiths/people/smondal/tower of hanoi/logs/gpt4 standard icl with error monitor, planner, try propose two moves with temperature, no previous step context, step by step external feedback/problem{}.log'.format(i+1), 'a') as w:
