...
import time
from five_shot_3numbers_examples import standard_prompt
import json
import os
import argparse
...
def extract_moves_configurations(start_config, gpt_full_response):
	...
	for l in range(len(lines)):
		...
		if 'Move' in lines[l] and 'from' in lines[l] and 'to' in lines[l] and (('A' in lines[l] and 'B' in lines[l] ) or ('C' in lines[l] and 'B' in lines[l] ) or ('A' in lines[l] and 'C' in lines[l] )):
			...
			while another_cur_try <5:
				try:
					...
					for m in range(len(response.choices[0].message.content) - 24):
						...
						if 'correct' in sub_str and 'sequence' in sub_str and 'moves' in sub_str:
